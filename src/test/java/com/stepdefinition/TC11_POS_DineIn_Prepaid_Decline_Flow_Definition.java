...
//		}
		...
		Thread.sleep(500);
		Map<java.lang.String, Float> grandCalculationSummary = grandCalculationSummary(menuPrice, entityPercentage);
		Thread.sleep(500);
		System.out.println(grandCalculationSummary);
		Thread.sleep(500);
		roundedCalculationSummary = roundCalculationSummary(grandCalculationSummary);
		Thread.sleep(500);
		System.out.println(roundedCalculationSummary);
		Thread.sleep(500);
		...
		if (!(percentageSaltx == 0)) {
			...
			Assert.assertEquals(salTxreplace,
					"Sales Tax$" + roundStringValue(roundedCalculationSummary.get("salesTax")));
		...
		if (!(percentageSerFee == 0)) {
			...
			Assert.assertEquals(serFeereplace,
					"Service Fee$" + roundStringValue(roundedCalculationSummary.get("serviceFee")));
		...
		if (SerFeeTXCheckBx.equals("ON")) {
			...
			Assert.assertEquals(serFeeTaxreplace,
					"Service Fee Tax$" + roundStringValue(roundedCalculationSummary.get("serviceFeeTax")));
				...
				if (Float.parseFloat(gratuityPercent) > 0) {
					if (subTotal >= Float.parseFloat(subTotalReaches)) {
						if (takeAwayCBx.equals("ON") || dineInCBx.equals("ON")) {// need to get current order type +
							...
							Assert.assertEquals(gratuityreplace,
									"Gratuity$" + roundStringValue(roundedCalculationSummary.get("gratuity")));
...
//need to get CURRENT ORDER TYPE + button takeAway & dineIn status - ####-Pass Global OrderType String-####
							...
							Assert.assertEquals(gratuityTaxreplace,
									"Gratuity Tax$" + roundStringValue(roundedCalculationSummary.get("gratuityTax")));
...
//need to get CURRENT ORDER TYPE + button takeAway & dineIn status - ####-Pass Global OrderType String-####
	...
	public void userShouldVerifyTheReceivePaymentPageCalculationAfterApplyingDiscount() throws IOException {
		...
		if (subTotal >= discountMinimumOrderAmt) {
			Map<String, Float> CalculationSummary = grandCalculationSummaryAfterDiscount(menuPrice, entityPercentage,
					excludedItems, discountPercentage);
		...
		if (!(percentageSerFee == 0)) {
			...
			Assert.assertEquals(serFeeRepTxt,
					"Service Fee$" + roundStringValue(roundCalculationSummary1.get("serviceFee")));
			System.out.println("ExecutedSF");
		...
		if (SerFeeTXCheckBx.equals("ON")) {
			...
			String serFeeTaxreplace = serFeeTxPD.replaceAll("[\\r\\n]+", "");
			Assert.assertEquals(serFeeTaxreplace,
					"Service Fee Tax$" + roundStringValue(roundCalculationSummary1.get("serviceFeeTax")));
			System.out.println("ExecutedSFT");
				...
				if (Float.parseFloat(gratuityPercent) > 0) {
					if (updatedSubTotal >= Float.parseFloat(subTotalReaches)) {
						if (takeAwayCBx.equals("ON") || dineInCBx.equals("ON")) {// need to get current order type +
							...
							String gratuityreplace = gratuityPD.replaceAll("[\\r\\n]+", "");
							Assert.assertEquals(gratuityreplace,
									"Gratuity$" + roundStringValue(roundCalculationSummary1.get("gratuity")));
							System.out.println("ExecutedGty");
...
//need to get CURRENT ORDER TYPE + button takeAway & dineIn status - ####-Pass Global OrderType String-####
							...
							if (gratuityTaxreplace.equals(
									"Gratuity Tax$" + roundStringValue(roundCalculationSummary1.get("gratuityTax")))) {

								Assert.assertEquals(gratuityTaxreplace, "Gratuity Tax$"
										+ roundStringValue(roundCalculationSummary1.get("gratuityTax")));
...
//			System.out.println(split[1]);
	...
	public void userShouldVerifyTheCustomerInformationField() throws InterruptedException {
		...
		Assert.assertEquals(null, nameTxt);
		Thread.sleep(500);
		// Upper Case
		pma.getPOS_FlowOne_POM().getCustomerNameTxtBxPaymentDetails().click();
		pma.getPOS_FlowOne_POM().getCustomerNameTxtBxPaymentDetails()
		...
		Assert.assertEquals(upperCase + ", Name", nameTxt1);
		Thread.sleep(500);
		// Lower Case
		pma.getPOS_FlowOne_POM().getCustomerNameTxtBxPaymentDetails().click();
		pma.getPOS_FlowOne_POM().getCustomerNameTxtBxPaymentDetails()
		...
		Assert.assertEquals(lowerCase + ", Name", nameTxt2);
		Thread.sleep(500);
		// Numbers
		pma.getPOS_FlowOne_POM().getCustomerNameTxtBxPaymentDetails().click();
		pma.getPOS_FlowOne_POM().getCustomerNameTxtBxPaymentDetails()
		...
		Assert.assertEquals(numbers + ", Name", nameTxt3);
		Thread.sleep(500);
		// Text
		pma.getPOS_FlowOne_POM().getCustomerNameTxtBxPaymentDetails().click();
...
//		   System.out.println(orderId);
			// String tableName =
			// pma.getPOS_FlowOne_POM().getTableNameRightside().getAttribute("content-desc");
//		   System.out.println(tableName);
...
//		
	...
	public void calculationValidation() {
		...
		Map<java.lang.String, Float> calculationSummaryAfterAddMenusAfterDiscount = grandCalculationSummaryAfterDiscount(
				menuItems, percentages, exclude, 21);
